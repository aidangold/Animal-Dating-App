...
export default function ViewPets() {
    ...
    function handleSave(petId) {
        const updatedPetSnakeCase = {};
        for (const key in editedPetData) {
            if (Object.hasOwnProperty.call(editedPetData, key)) {
                const snakeCaseKey = key.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`);
                updatedPetSnakeCase[snakeCaseKey] = editedPetData[key];
            }
        }

        console.log('Updating pet with ID:', petId);
        console.log('Updated pet data:', updatedPetSnakeCase);
        ...
        <tr key={pet.petID}>
            <td>
                ...
                {editedPetId === pet.petID && (
                    <div>
                        <input type="file" accept="image/*" onChange={(e) => handlePhotoChange(e, pet.petID)} />
                    </div>
                )}
            </td>
            <td>{pet.petID}</td>
            <td>
                {editedPetId === pet.petID ? (
                    <input type="text" value={editedPetData.petName || pet.petName} onChange={(e) => handleChange(e, 'petName')} />
                ) : pet.petName}
            </td>
            <td>
                {editedPetId === pet.petID ? (
                    <input type="text" value={editedPetData.petType || pet.petType} onChange={(e) => handleChange(e, 'petType')} />
                ) : pet.petType}
            </td>
            <td>
                {editedPetId === pet.petID ? (
                    <input type="text" value={editedPetData.petSex || pet.petSex} onChange={(e) => handleChange(e, 'petSex')} />
                ) : pet.petSex}
            </td>
            <td>
                {editedPetId === pet.petID ? (
                    <input type="text" value={editedPetData.petBreed || pet.petBreed} onChange={(e) => handleChange(e, 'petBreed')} />
                ) : pet.petBreed}
            </td>
            <td>
                {editedPetId === pet.petID ? (
                    <select value={editedPetData.petAvailability || pet.petAvailability} onChange={(e) => handleChange(e, 'petAvailability')}>
            ...
            <td>
                {editedPetId === pet.petID ? (
                    <IconButton aria-label="save" size="large" onClick={() => handleSave(pet.petID)}>
                        <SaveRoundedIcon />
                    </IconButton>
                ) : (
                    <IconButton aria-label="edit" size="large" onClick={() => handleEdit(pet.petID)}>
                        <EditRoundedIcon />
                    </IconButton>
                )}
            </td>
            <td>
                <IconButton aria-label="delete" size="large" onClick={() => handleDelete(pet.petID)}> {}
                    <DeleteRoundedIcon />
                </IconButton>
            </td>
        </tr>
